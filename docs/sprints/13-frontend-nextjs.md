...
| `SourceList`'s relevance filter | Drops any source whose displayed relevance rounds to 0% before rendering the list | The backend (Sprint 9's `Reranker`) now filters these out itself, but this is a defensive second layer — showing Sarah a source at 0% would actively undermine trust in the ones that are genuinely relevant | Trusting every source the API happens to return, unfiltered, means a frontend-only bug or an edge case in the backend fix could still surface an obviously-irrelevant result |
