...
| 31 | An unrelated uploaded document showed up in the sources list at 0% relevance, alongside the genuinely correct match, for a question that had nothing to do with it | `Reranker.rerank()` always returned exactly the top 3 chunks by score — `settings.reranker_score_threshold` (0.20) had existed in config since this was first built, but nothing ever actually read it. With only 1 truly relevant chunk available, the other 2 slots got filled with whatever scored next highest, however weak — including a chunk from a completely different document. That chunk wasn't just UI clutter; it was also sitting in the LLM's context as noise on every matching request | Filter out anything below `reranker_score_threshold` *before* the `[:3]` cut, so "nothing else is relevant" can mean 1 source, or 0, instead of always exactly 3. Added a matching defensive filter in the frontend's `SourceList` too. Verified live: the same two-document setup (a real leave policy + an unrelated document) went from 3 sources (one at 0%) to exactly 1 | [Sprint 9](09-rag-pipeline-and-hybrid-search.md), [Sprint 13](13-frontend-nextjs.md) |
